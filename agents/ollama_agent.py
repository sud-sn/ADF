...
# ---------------------------------------------------------------------------
# DFS (Data Flow Script) specific system prompt
# ---------------------------------------------------------------------------

_DFS_SYSTEM_PROMPT = """\
You are a specialist ADF Mapping Data Flow Script (DFS) to PySpark migration engineer.
Your ONLY task: translate DFS expressions into Python 3.10+ / PySpark code.
...
- Column reference: bare name     → col("name")

NESTED EXPRESSIONS:
- iif(equals(A, '0'), 0, iif(equals(A, B), C, 777))
  → when(col("A") == lit("0"), lit(0)).when(col("A") == col("B"), col("C")).otherwise(lit(777))
- iif(isNull(A) || trim(A) == '', '0', A)
...
def get_system_prompt(mode: str = "pipeline") -> str:
    """Return the appropriate system prompt for the given mode."""
    if mode == "dataflow":
        return _DFS_SYSTEM_PROMPT
