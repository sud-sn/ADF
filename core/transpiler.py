...
def _transpile_mapping_data_flow(activity: MappingDataFlowActivity) -> ActivityTranspileResult:
    """Map a MappingDataFlowActivity to PySpark DataFrame transformation scaffold."""
    var = _safe_var(activity.name)
    props = activity.parsed_type_properties
    expressions = _extract_expressions(activity.type_properties)

    dataflow_name = props.dataflow.reference_name if props else "unknown_dataflow"
    dataflow_params = props.dataflow.parameters if props else {}

    # Compute configuration for comments
    compute_type = "General"
    core_count = 8
    if props and props.compute:
        compute_type = props.compute.compute_type or "General"
        core_count = props.compute.core_count or 8

    # Staging info
    staging_info = ""
    if props and props.staging and props.staging.linked_service:
        staging_ls = props.staging.linked_service.reference_name
        staging_folder = props.staging.folder_path or "staging"
        staging_info = f"""\n# Staging: linked_service={staging_ls}, folder={staging_folder}"""
    ...
    if dataflow_params:
        param_entries = "\n".join(
            f'    "{k}": {repr(v)},'
            for k, v in dataflow_params.items()
        )
        param_lines = f"""\n# Data Flow Parameters\ndataflow_params_{var}: dict = {{\n{param_entries}\n}}"""

    trace_level = "none"
    if props and props.trace_level:
        trace_level = props.trace_level

    code = f'''\
# --- MappingDataFlow: {activity.name} ---
# Data Flow        : {dataflow_name}
# Compute          : {compute_type} ({core_count} cores)
# Trace Level      : {trace_level}{staging_info}

logger.info("Starting Mapping Data Flow: {activity.name} (dataflow={dataflow_name})")
{param_lines}

# -----------------------------------------------------------------------
# Source: Read input data
# TODO: Configure source connection from data flow definition '{dataflow_name}'
# -----------------------------------------------------------------------
df_{var}_source = (
    spark.read
    .format("parquet")  # TODO: adjust format based on data flow source type
    # .option("path", "<source_path>")
    .load()
)

logger.info("Data Flow [{activity.name}] source read complete. Row count: %d", df_{var}_source.count())

# -----------------------------------------------------------------------
# Transformations: Apply data flow transformation logic
# TODO: Translate transformation steps from data flow '{dataflow_name}':
#   - Derived columns   → .withColumn("col", expr(...))
#   - Filters           → .filter(...) / .where(...)
#   - Joins             → df_a.join(df_b, on=..., how=...)
#   - Aggregates        → .groupBy(...).agg(...)
#   - Select / Rename   → .select(...) / .withColumnRenamed(...)
#   - Sort              → .orderBy(...)
#   - Window functions  → Window.partitionBy(...).orderBy(...)
#   - Conditional split → multiple .filter() branches
#   - Union             → df_a.unionByName(df_b)
#   - Lookup            → df_a.join(df_lookup, ..., "left")
#   - Exists            → df_a.join(df_check, ..., "left_semi")
#   - Pivot / Unpivot   → .groupBy(...).pivot(...).agg(...)
# -----------------------------------------------------------------------
df_{var}_transformed = df_{var}_source  # TODO: apply transformation chain

# -----------------------------------------------------------------------
# Sink: Write output data
# TODO: Configure sink connection from data flow definition '{dataflow_name}'
# -----------------------------------------------------------------------
(
    df_{var}_transformed.write
    .format("parquet")  # TODO: adjust format based on data flow sink type
    .mode("overwrite")
    # .option("path", "<sink_path>")
    .save()
)

logger.info("Data Flow [{activity.name}] sink write complete.")
'''
...
class ActivityTranspiler:
    ...
    def _dispatch(self, activity: ActivityBase) -> ActivityTranspileResult:
        ...
        if isinstance(activity, MappingDataFlowActivity):
            return _transpile_mapping_data_flow(activity)
