...
class DataFlowReference(BaseModel):
    """Reference to an ADF Data Flow resource."""
    reference_name: str = Field(..., alias="referenceName")
    type: str = Field(default="DataFlowReference")
    parameters: dict[str, Any] = Field(default_factory=dict)
    dataset_parameters: dict[str, Any] = Field(default_factory=dict, alias="datasetParameters")

    model_config = {"populate_by_name": True, "extra": "allow"}


class DataFlowStagingInfo(BaseModel):
    """Staging linked service configuration for data flow execution."""
    linked_service: LinkedServiceReference | None = Field(default=None, alias="linkedService")
    folder_path: str | None = Field(default=None, alias="folderPath")

    model_config = {"populate_by_name": True, "extra": "allow"}


class DataFlowComputeType(BaseModel):
    """Compute configuration for data flow execution."""
    compute_type: str | None = Field(default=None, alias="computeType")
    core_count: int | None = Field(default=None, alias="coreCount")

    model_config = {"populate_by_name": True, "extra": "allow"}


class MappingDataFlowTypeProperties(BaseModel):
    """Type properties specific to MappingDataFlow activities."""
    dataflow: DataFlowReference
    staging: DataFlowStagingInfo | None = Field(default=None)
    compute: DataFlowComputeType | None = Field(default=None)
    trace_level: str | None = Field(default=None, alias="traceLevel")
    continue_on_error: bool | None = Field(default=None, alias="continueOnError")
    run_concurrently: bool | None = Field(default=None, alias="runConcurrently")
    source_staging_concurrency: int | None = Field(default=None, alias="sourceStagingConcurrency")

    model_config = {"populate_by_name": True, "extra": "allow"}


class MappingDataFlowActivity(ActivityBase):
    """Strongly-typed model for ADF Mapping Data Flow activities."""

    type: str = Field(default=ActivityType.MAPPING_DATA_FLOW.value)
    parsed_type_properties: MappingDataFlowTypeProperties | None = Field(default=None, exclude=True)

    @model_validator(mode="after")
    def parse_type_properties(self) -> "MappingDataFlowActivity":
        if self.type_properties:
            try:
                self.parsed_type_properties = MappingDataFlowTypeProperties.model_validate(
                    self.type_properties
                )
            except Exception as exc:
                logger.warning(
                    "MappingDataFlowActivity '%s': typeProperties validation failed — %s",
                    self.name,
                    exc,
                )
        return self
...
AnyActivity = Annotated[
    Union[CopyActivity, ForEachActivity, IfConditionActivity, MappingDataFlowActivity, GenericActivity],
...
MappingDataFlowActivity.model_rebuild()
...
_ACTIVITY_TYPE_MAP: dict[str, type[ActivityBase]] = {
    ...
    ActivityType.MAPPING_DATA_FLOW.value: MappingDataFlowActivity,
}
